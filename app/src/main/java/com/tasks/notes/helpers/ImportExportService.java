...
import com.tasks.notes.storage.StorageProvider;
...
public class ImportExportService implements Closeable {

    private static ImportExportService instance;

    public static ImportExportService getInstance(ListFragment fragment) {
        if (instance == null) {
            instance = new ImportExportService(fragment);
        }
        return instance;
    }
    ...
    private final StorageProvider databaseProvider;
    ...
    private ImportExportService(ListFragment fragment) {
        this.fragmentRef = new WeakReference<>(fragment);
        contentResolver = fragment.getContext().getContentResolver();
        databaseProvider = DatabaseProvider.getInstance(fragment.getContext());
                    ...
                    case ACTION_IMPORT: {
                        ...
                        if (fragment != null) {
                            ...
                            Toast.makeText(fragment.getContext(),
                                    msg.getData().getString(ARG_STATUS),
                                    Toast.LENGTH_SHORT).show();
                        }

                        if (msg.obj != null) {
                            NotificationWrapper notification = (NotificationWrapper) msg.obj;
                            notification.close();
                        }
                        break;
                    }
    ...
    private static synchronized void importNotes(ContentResolver resolver,
                                                 StorageProvider provider, Uri uri)
